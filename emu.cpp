...
void mem_dump(int poc, ofstream& trcfile)
{
	...
	for (int i = 0; i < poc; i++)
	{
		...
		if(i%4)
		{   
            cout << int_hex_conv(memory[i]) << " ";
			trcfile << int_hex_conv(memory[i]) << " ";
        }
        else
		{
			cout << endl << endl << int_hex_conv(i) << "\t" << int_hex_conv(memory[i]) << " ";
			trcfile << endl << endl << int_hex_conv(i) << "\t" << int_hex_conv(memory[i]) << " ";
		}
    }
	cout << endl;
}
...
void mot_init()
{
	mot[0] = string("ldc"); 	mot[1] = string("adc");
	mot[2] = string("ldl"); 	mot[3] = string("stl");
	mot[4] = string("ldnl");   	mot[5] = string("stnl");
	mot[6] = string("add"); 	mot[7] = string("sub");
	mot[8] = string("shl"); 	mot[9] = string("shr");
	mot[10] = string("adj");   	mot[11] = string("a2sp");
	mot[12] = string("sp2a");	mot[13] = string("call");
	mot[14] = string("return");	mot[15] = string("brz");
	mot[16] = string("brlz");	mot[17] = string("br");
	mot[18] = string("HALT");	mot[19] = string("data");
	mot[20] = string("SET");
}
...
int trace(int PC, int poc, ofstream& trcfile)
{
	...
	while(true)
	{
		...
		cout << "PC: " << int_hex_conv(PC) << "\tSP: " << int_hex_conv(SP) << "\tA: " 
		<< int_hex_conv(A) << "\tB: " << int_hex_conv(B) << "\t" << mot[operation] 
		<< " " <<operand << endl << endl;

		trcfile << "PC: " << int_hex_conv(PC) << "\tSP: " << int_hex_conv(SP) << "\tA: " 
		<< int_hex_conv(A) << "\tB: " << int_hex_conv(B) << "\t" << mot[operation] 
		<< " " <<operand << endl << endl;
